...
class gtm:
    ...
    def fit(self, input_dataset):
        ...
        #print(distance_between_map_and_rbf_grids)
        #print(self.phi_of_map_rbf_grids)
        #print(np.linalg.matrix_rank(self.phi_of_map_rbf_grids))
        #print(self.phi_of_map_rbf_grids.shape)
    ...
    def responsibility(self, input_dataset):
        ...
        #self.likelihood_value = (np.log((self.beta / 2.0 / np.pi) ** (input_dataset.shape[1] / 2.0) /
        #                                np.prod(self.shape_of_map) * (rbf_for_responsibility.sum(axis=1)+1.0e-15))).sum()

        #print(np.prod(self.shape_of_map))
        #print((self.beta / 2.0 / np.pi) ** (input_dataset.shape[1] / 2.0) /
        #                                np.prod(self.shape_of_map) * rbf_for_responsibility.sum(axis=1))
        #print(rbf_for_responsibility)
        #if np.prod(self.shape_of_map) * rbf_for_responsibility.sum(axis=1) == 0:
        #print(self.likelihood_value)
